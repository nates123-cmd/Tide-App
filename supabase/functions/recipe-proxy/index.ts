...
// Walk JSON-LD blob in any shape (object, array, @graph) and collect Recipe nodes.
function collectRecipes(node, sink) {
  if (!node) return;
  if (Array.isArray(node)) { node.forEach((n) => collectRecipes(n, sink)); return; }
  if (typeof node !== 'object') return;
  const t = node['@type'];
  if (t === 'Recipe' || (Array.isArray(t) && t.includes('Recipe'))) sink.push(node);
  if (node['@graph']) collectRecipes(node['@graph'], sink);
}

function extractServings(r) {
  const candidates = [r.recipeYield, r.yield];
